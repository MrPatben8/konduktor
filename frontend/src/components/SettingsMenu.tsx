...
export function SettingsMenu({ onOpenPathMapping, up = false }: Props) {
  ...
  const item =
    'flex w-full flex-col items-start rounded-[10px] px-2.5 py-1.5 text-left hover:bg-ink-800'

  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="btn-glass flex h-10 w-10 items-center justify-center rounded-full text-muted hover:text-text"
        title="Settings"
        aria-label="Settings"
      >
        <svg
      ...
      {open && (
        <div
          className={`glass-overlay absolute right-0 z-30 w-60 !rounded-2xl p-1.5 ${
