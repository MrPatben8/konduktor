...
Konduktor's source of truth is the library file; this module additionally syncs the
...
# Whose POPM frame we claim when writing an ID3 rating. The frame is per-owner,
# so writing under the DJ app's own email is what makes the rating visible there.
# The *51 scaling below is the POPM convention itself, not a platform quirk.
DEFAULT_POPM_EMAIL = "no@email"
...
def write_tags(
    path: Path, meta: TagMeta, *, popm_email: str = DEFAULT_POPM_EMAIL
) -> TagResult:
    if not path.exists():
        return TagResult(False, "file-not-found", str(path))
    ext = path.suffix.lower()
    try:
        if ext == ".mp3":
            _write_id3(path, meta, id3_only=False, popm_email=popm_email)
        elif ext in (".aif", ".aiff"):
            _write_aiff(path, meta, popm_email=popm_email)
...
# ---- ID3 (mp3 / aiff) --------------------------------------------------
def _apply_id3(id3, meta: TagMeta, popm_email: str = DEFAULT_POPM_EMAIL) -> None:
...
def _write_id3(
    path: Path, meta: TagMeta, id3_only: bool, popm_email: str = DEFAULT_POPM_EMAIL
) -> None:
    from mutagen.id3 import ID3, ID3NoHeaderError
    ...
    try:
        ...
        id3 = ID3()
    _apply_id3(id3, meta, popm_email)
    id3.save(path)


def _write_aiff(path: Path, meta: TagMeta, popm_email: str = DEFAULT_POPM_EMAIL) -> None:
    from mutagen.aiff import AIFF

    audio = AIFF(path)
    if audio.tags is None:
        audio.add_tags()
    _apply_id3(audio.tags, meta, popm_email)
    audio.save()
